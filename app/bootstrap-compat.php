...
add_action( 'after_switch_theme', 'mag_switch_theme'   );
add_action( 'load-customize.php', 'mag_load_customize' );
add_action( 'template_redirect',  'mag_preview'        );
 ...
 * @return string
 */
function mag_compat_message() {

	if ( version_compare( $GLOBALS['wp_version'], '4.9.6', '<' ) ) {

		return sprintf(
			// Translators: 1 is the required WordPress version and 2 is the user's current version.
			__( 'Mag requires at least WordPress version %1$s. You are running version %2$s. Please upgrade and try again.' ),
			'4.9.6',
			$GLOBALS['wp_version']
		);
		...
		return sprintf(
			...
			__( 'Mag requires at least PHP version %1$s. You are running version %2$s. Please upgrade and try again.' ),
			'5.6',
			PHP_VERSION
		);
	}
 ...
 * @return void
 */
function mag_switch_theme( $old_name ) {

	switch_theme( $old_name ? $old_name : WP_DEFAULT_THEME );

	unset( $_GET['activated'] );

	add_action( 'admin_notices', 'mag_upgrade_notice' );
}
 ...
 * @return void
 */
function mag_upgrade_notice() {

	printf( '<div class="error"><p>%s</p></div>', esc_html( mag_compat_message() ) );
}
 ...
 * @return void
 */
function mag_load_customize() {

	wp_die( esc_html( mag_compat_message() ), '', array( 'back_link' => true ) );
}
 ...
 * @return void
 */
function mag_preview() {

	if ( isset( $_GET['preview'] ) ) { // WPCS: CSRF ok.
		wp_die( esc_html( mag_compat_message() ) );
	}
}
